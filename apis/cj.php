...
class Affiliate_Power_Api_Cj {  
    ...
    static public function addSubId($link, $subid) {
        ...
        if (strpos($link, 'sid=')) $link = preg_replace('@sid=[0-9a-z\-_]+@i', 'sid='.$subid, $link);
        else {
            if (strpos($link, 'url=') !== false) $link = str_replace('url=', 'sid='.$subid.'&url=', $link);
            elseif (strpos($link, "?") !== false) $link .= '&sid='.$subid;
            else $link .= '?sid='.$subid;
        }
	...
	static public function downloadTransactions($fromTS, $tillTS) {
	    ...
	    if (!isset($options['cj-id']) || !isset($options['cj-key'])) return array();
	    $website_id = $options['cj-id'];
	    $auth_key = $options['cj-key'];
	
		$fromTS_temp = $fromTS;
		$tillTS_temp = min($fromTS + 3600*24*25, $tillTS); //cj does not allow more than 30 days.
		...
		while ($tillTS_temp <= $tillTS) {
			...
			$tillTS_temp = min($tillTS_temp+3600*24*10, $tillTS);
		}
